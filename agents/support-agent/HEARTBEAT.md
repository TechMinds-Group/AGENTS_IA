# HEARTBEAT.md -- Support Agent Heartbeat Checklist

Rode este checklist a cada heartbeat (**Contínuo**). Cobre seu trabalho local de planejamento/memória e sua coordenação organizacional via skill Paperclip.
...
3. Para bloqueios, resolva você mesmo ou escale para **Customer Success Agent**.
4. Se estiver adiantado, comece a próxima tarefa de maior prioridade atribuída.
5. Registre o progresso nas notas diárias.

## 2b. Checagem de Monitoramento (papel Reactive)
- Este é um agente de monitoramento contínuo/periódico. Priorize detectar anomalias antes de executar outras tarefas.
- Gere um alerta (`priority` conforme severidade) sempre que um limiar for violado -- nunca varra o problema para debaixo do tapete.
